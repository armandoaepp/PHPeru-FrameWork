...
class CixPHP
{
    ...
    PROPIEDAR PRIVADA DE Armando Enrique Pisfil Puemape (@armandoaepps) codigo abierto 
                ...
                for ($i = $ini; $i < $fin; $i++) {
                    ...
                    for ($z = 0; $z < count($data["columnas"]); $z++) {
                        ...
                        if (isset($data["cuerpo"][$i]["$columnas"])) {
                            if ($pactu != "") {
                                if ($conta == 0) {
                                    $tabla .= '<td><a href="' . $pactu . 'id=' . $data["cuerpo"][$i]["$columnas"] . '&accion=Mactualizar"><img src="../img/icon-edit.png" title="Actualizar" width="16" height="16"/></a></td>';
                                }
                            }
                            if ($peli != "") {
                                if ($conte == 0) {
...
function Paginar_PHP($cantele, $crxpag, $paginap, $cantlink, $orderby = -1, $buscar = "")
{
    ...
    $numOrden   = "";
    if ($buscar != "")
        $textBuscar = "&busca=" . $buscar . "&accion=Listar";
    if ($orderby != -1)
