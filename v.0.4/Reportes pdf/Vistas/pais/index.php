...
break;
case "Exportar":
 $NomnbreArchivo="Lista_pais";
 $data=CListadoSimple_pais();
if(count($data["cuerpo"])>0){
 $data_pais=$cix->ImprimeTable("tablapais",$data,"","","table");
 include_once "../cixexportar.php"; 
 if(isset($_GET["type"])){
   $tipo=$_GET["type"];
     switch($tipo){
       case "pdf":
         Exportar_PDF( $data_pais);
       break;
       case "xls":
         Exportar_EXCEL( $data_pais,$NomnbreArchivo);
       break;
       case "doc":
         Exportar_WORD( $data_pais,$NomnbreArchivo);
       break;
       case "html":
         Exportar_HTML( $data_pais,$NomnbreArchivo);
       break;
     }
 }
}
