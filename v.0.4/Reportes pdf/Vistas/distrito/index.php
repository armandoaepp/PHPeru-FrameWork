...
break;
case "Exportar":
 $NomnbreArchivo="Lista_distrito";
 $data=CListadoSimple_distrito();
if(count($data["cuerpo"])>0){
 $data_distrito=$cix->ImprimeTable("tablapais",$data,"","","table");
 include_once "../cixexportar.php"; 
 if(isset($_GET["type"])){
   $tipo=$_GET["type"];
     switch($tipo){
       case "pdf":
         Exportar_PDF( $data_distrito);
       break;
       case "xls":
         Exportar_EXCEL( $data_distrito,$NomnbreArchivo);
       break;
       case "doc":
         Exportar_WORD( $data_distrito,$NomnbreArchivo);
       break;
       case "html":
         Exportar_HTML( $data_distrito,$NomnbreArchivo);
       break;
     }
 }
}
