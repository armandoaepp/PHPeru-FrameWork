...
function generandoIndex($atri, $cabeza, $tabla, $tablaref, $arrayenlace, $arrayenlace2)
{
    ...
    if (!empty($tabla)) {
        ...
        $texto .= ' require_once "../layout.php";' . PHP_EOL;
        $texto .= 'break;' . PHP_EOL;
        // para exportar a diferente formatos
        $texto .= 'case "Exportar":' . PHP_EOL;             
        $texto .= ' $NomnbreArchivo="Lista_' . $tabla . '";' . PHP_EOL;
        $texto .=' $data=CListadoSimple_' . $tabla . '();'. PHP_EOL;
        $texto .='if(count($data["cuerpo"])>0){'. PHP_EOL;
        $texto .=' $data_' . $tabla . '=$cix->ImprimeTable("tablapais",$data,"","","table");'. PHP_EOL;
        $texto .=' include_once "../cixexportar.php"; '. PHP_EOL;        
        $texto .=' if(isset($_GET["type"])){'. PHP_EOL;
        $texto .='   $tipo=$_GET["type"];'. PHP_EOL;
        $texto .='     switch($tipo){'. PHP_EOL;
        $texto .='       case "pdf":'. PHP_EOL;
        $texto .='         Exportar_PDF( $data_'. $tabla.');'. PHP_EOL;
        $texto .='       break;'. PHP_EOL;
        $texto .='       case "xls":'. PHP_EOL;
        $texto .='         Exportar_EXCEL( $data_'. $tabla.',$NomnbreArchivo);'. PHP_EOL;
        $texto .='       break;'. PHP_EOL;
        $texto .='       case "doc":'. PHP_EOL;
        $texto .='         Exportar_WORD( $data_'. $tabla.',$NomnbreArchivo);'. PHP_EOL;
        $texto .='       break;'. PHP_EOL;
        $texto .='       case "html":'. PHP_EOL;
        $texto .='         Exportar_HTML( $data_'. $tabla.',$NomnbreArchivo);'. PHP_EOL;
        $texto .='       break;'. PHP_EOL;
